...
from soaphound.ad.collectors.certtemplates import collect_cert_templates, format_cert_templates
...
"""
    ...
    # Collect and format Certificate Templates (PKI)
    logging.info("Collecting Certificate Templates...")
    cert_templates = collect_cert_templates(options.domain_controller, options.domain, options.username, auth, base_dn_override=default_dn)
    cert_templates_bh = format_cert_templates(cert_templates, options.domain, domain_sid, id_to_type_cache, value_to_id_cache, objecttype_guid_map)
    ...
    safe_export_bloodhound_json(cert_templates_bh, os.path.join(output_dir, timestamp + "certtemplates.json"))
